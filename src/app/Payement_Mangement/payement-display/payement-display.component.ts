...
@Component({
  selector: 'app-payement-display',
  templateUrl: './payement-display.component.html',
  styleUrls: ['./payement-display.component.css']
})
export class PayementDisplayComponent {
  ...
  getEvents()
  {
    this.loading=true
    // @ts-ignore
    this.payementService.getPayements(sessionStorage.getItem("userId")).pipe(  map((response: any) => response as DataItem[]))
      .subscribe(
        (value) =>
        {
          console.log(value)
